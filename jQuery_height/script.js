...
$(document).ready(function(){
    ...
    $.each($('.character, .chatbubble'), function(iteration, element){
        ...
        chatHeight = parseFloat($(element).css('height'));
        $(element).data('data-height', chatHeight + 'px');
    })
})
...
$('#formControlRangeX').on('input', function(event){
    ...
    $.each($('.chatbubble'), function(index, object){
        // Change the height of the chatbubble
        $(object).height(parseFloat($(object).data('data-height')) + parseFloat(event.currentTarget.value)/10 + 'px');
    })
})