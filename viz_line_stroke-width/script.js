...
document.addEventListener("DOMContentLoaded", function(){
    ...
    // If the user changes the X slider, adjust the size of the chart line.
    $('#formControlRangeX').on('input', function(event){
        // Get the input from the slider.
        userVal = parseFloat($(event.currentTarget).prop('value'));

        // Adjust the size of the stroke drawn for the chart line.
        body.selectAll('path')
            .attr('stroke-width', (0.2 + userVal/100) +'vw')
    })
})
