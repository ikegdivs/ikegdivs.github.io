...
//  Constants
//-----------------------------------------------------------------------------
const MIN_DELTA_Z = 0.001; // minimum elevation change for conduit
                                         // slopes (ft)
...
function link_readParams(j, type, k, tok, ntoks)
//
//  Input:   j     = link index
//           type  = link type code
//           k     = link type index
...
function link_readXsectParams(tok, ntoks)
//
...
function link_readLossParams(tok, ntoks)
//
//  Input:   tok[] = array of string tokens
//           ntoks = number of tokens
//  Output:  returns an error code
...
function  link_setParams(j, type, n1, n2, k, x)
//
//  Input:   j   = link index
//           type = link type code
...
function link_convertOffsets(j)
//
//  Input:   j = link index
//  Output:  none
...
function link_getOffsetHeight(j, offset, elev)
//
//  Input:   j = link index
...
function link_setOldHydState(j)
//
//  Input:   j = link index
//  Output:  none
...
function link_setOldQualState(j)
//
//  Input:   j = link index
//  Output:  none
...
function link_setTargetSetting(j)
//
//  Input:   j = link index
//  Output:  none
...
function link_setSetting(j, tstep)
//
//  Input:   j = link index
...
function link_setFlapGate(j, n1, n2, q)
//
//  Input:   j = link index
...
function link_getResults(j, f, x)
//
//  Input:   j = link index
...
function link_setOutfallDepth(j)
//
//  Input:   j = link index
//  Output:  none
...
function link_getVelocity(j, flow, depth)
//
//  Input:   j     = link index
...
function link_getLossRate(j, q)                                       //(5.1.014)
//
//  Input:   j = link index
//           q = flow rate (ft3/sec)
//           tstep = time step (sec)
...
function  link_getFullState(a1, a2, aFull)
//
...
function  conduit_readParams(j, k, tok, ntoks)
//
//  Input:   j = link index
//           k = conduit index
//           tok[] = array of string tokens
...
function conduit_getLengthFactor(j, k, roughness)
//
//  Input:   j = link index
//           k = conduit index
...
function  conduit_initState(j, k)
//
//  Input:   j = link index
//           k = conduit index
//  Output:  none
...
function conduit_getLossRate(j, q)
//
//  Input:   j = link index
...
function  pump_readParams(j, k, tok, ntoks)
//
//  Input:   j = link index
//           k = pump index
//           tok[] = array of string tokens
...
function  orifice_readParams(j, k, tok, ntoks)
//
//  Input:   j = link index
//           k = orifice index
//           tok[] = array of string tokens
...
function  orifice_setSetting(j, tstep)
//
//  Input:   j = link index
...
function orifice_getWeirCoeff(j, k, h)
//
//  Input:   j = link index
//           k = orifice index
...
function orifice_getFlow(j, k, head, f, hasFlapGate)
//
//  Input:   j = link index
//           k = orifice index
...
function   weir_readParams(j, k, tok, ntoks)
//
//  Input:   j = link index
//           k = weir index
//           tok[] = array of string tokens
...
function weir_getFlow(j, k, head, dir, hasFlapGate,
                   q1,  q2)
//
//  Input:   j    = link index
//           k    = weir index
...
function weir_getOrificeFlow(j, head, y, cOrif)
//
//  Input:   j = link index
//           head = head across weir (ft)
...
function  weir_getdqdh(k, dir, h, q1, q2)
{
    let q1h;
    let q2h;
...
function outlet_readParams(j, k, tok, ntoks)
//
//  Input:   j = link index
//           k = outlet index
//           tok[] = array of string tokens
...
function outlet_getFlow(k, head)
//
